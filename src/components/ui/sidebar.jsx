  ...
  // Mejorar la detección de children
  ...
  // Buscar texto: puede ser string directo O un span con texto
  const textChild = childrenArray.find(
    child => {
      if (typeof child === "string") return true
      if (React.isValidElement(child) && child.type === "span") return true
      return false
    }
  )

  // Extraer el texto real
  const textContent = typeof textChild === "string" 
    ? textChild 
    : (React.isValidElement(textChild) ? textChild.props.children : null)

  // DEBUG: Descomentar estas líneas para ver qué está pasando
  // console.log('State:', state)
  // console.log('Children:', childrenArray)
  // console.log('Text child:', textChild)
  ...
  const button = (
    ...
    >
      ...
      {/* Mostrar el texto solo cuando NO está colapsado */}
      {state === "expanded" && textContent && (
        <span className="truncate transition-all duration-200 ml-1">
          {textContent}
        </span>
      )}
    </Comp>
